...
class MPC_Sim:
    def __init__(self, sim_main_params, logs_path):
        ...
        self.simMode = self.sim_main_params[
            "simMode"
        ]  # 0 -> CiL (MPC + simulation model: separate dynamics model)     |   1 -> MPC Sim (MPC + simulation model: MPC predictions)
        ref_trajectory_type = self.sim_main_params[
            "ref_trajectory_type"
        ]  # 0: smooth continuous reference trajectory, 1: discrete set of trajectories

        self.Tp = self.sim_main_params["Tp"]  # prediction horizon [s]
        self.Ts_MPC = self.sim_main_params[
            "Ts_MPC"
        ...
        idx_ref_start = self.sim_main_params[
            "idx_ref_start"
        ...
        if ref_trajectory_type == 0:
            ...
            self.current_pose[2] = postprocess_yaw(
                self.ref_traj_set["ref_yaw"][idx_ref_start]
            )
    ...
    def set_Vehicle(self, Vehicle):
        ...
        self.veh_length, self.veh_width = (
            self.Vehicle.params.veh_length,
            self.Vehicle.params.veh_width,
        )

        # --- Simulator Disturbance setup
        self.disturbances_state_derivatives_sim = self.sim_main_params[
        ...
        if self.disturbances_state_derivatives_sim or self.state_estimation_sim:
            (
                self.playback_disturbance,
                self.disturbance_range_derivatives,
                self.disturbance_range_state_estimation,
                self.disturbance_bounds_derivatives,
                self.disturbance_bounds_state_estimation,
                self.disturbance_types,
                self.sim_disturbance_derivatives,
                self.sim_disturbance_state_estimation,
            ) = initDisturbanceSim(
                self.sim_main_params, self.logs_path, self.Nsim, self.nx_sim
            )
        else:
            self.sim_disturbance_derivatives, self.sim_disturbance_state_estimation = (
                np.zeros((self.Nsim, self.nx_sim)),
                np.zeros((self.Nsim, self.nx_sim)),
            )
    ...
    def sim_step(self, i, x, u0, pred_X):
        ...
        if self.simMode == 1:
            ...
            sim_model_control_input = np.array(
                [acceleration_input, steering_rate_input]
            )
            ...
            if not self.disturbances_state_derivatives_sim:
                ...
                if self.playback_disturbance:
                    ...
                    disturbances_derivatives = generate_disturbances(
                        self.disturbance_bounds_derivatives, self.disturbance_types[0]
                    )
                x_next_sim_disturbed = self.Vehicle.simulator_step_disturbed(
                    x,
                    np.concatenate(
                        (sim_model_control_input, np.array(disturbances_derivatives)),
                        axis=0,
                    ),
                )
            ...
            if self.state_estimation_sim:
                if self.playback_disturbance:
                    disturbances_state_estimation = (
                        self.sim_disturbance_state_estimation[i, :]
                    )
                else:
                    disturbances_state_estimation = generate_disturbances(
                        self.disturbance_bounds_state_estimation,
                        self.disturbance_types[1],
                    )
                x_next_sim_disturbed = [
                    x_next_sim_disturbed[j] + disturbances_state_estimation[j]
                    for j in range(len(x_next_sim_disturbed))
                ]
                # log disturbance
                for j in range(self.nx_sim):
                    self.sim_disturbance_state_estimation[i, j] = (
                        disturbances_state_estimation[j]
                    )
            # add acceleration state
            x_next = np.append(x_next_sim_disturbed, acceleration_input)
            self.current_pose[0] = x_next_sim[0]  # x
            self.current_pose[1] = x_next_sim[1]  # y
            self.current_pose[2] = x_next_sim[2]  # theta
            self.current_pose[3] = np.sqrt(
                x_next_sim[3] ** 2 + x_next_sim[4] ** 2
            )  # v_lon
    ...
    def StateEstimation(self, x_next):
        for i in range(len(x_next)):
            ...
            x_next[i] = moving_average_filter(
                np.array(self.x_buffer[i]), self.window_sizes[i]
            )
        return x_next
